...
var sequelize = new Sequelize('classroom', 'root', 'root', {//通过new关键字进行实例化，实例化后就会以连接池的形式连接到所使用的数据库，var sequelize = new Sequelize(database, [username=null], [password=null], [options={}])，实例化Sequelize时，需要传入数据库名、用户名和密码，还可以传入一个可选的options参数对象。
